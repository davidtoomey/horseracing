...
contract HorseBreeding is HorseOwnership {
    ...
    /// @dev Checks that a given newHorse is able to breed. Requires that the
    ///  current cooldown is finished (for sires) and also checks that there is
    ///  no pending pregnancy.
    function _isReadyToBreed(Horse _horsey) internal view returns (bool) {
        // In addition to checking the cooldownEndTime, we also need to check to see if
        // the cat has a pending birth; there can be some period of time between the end
        // of the pregnacy timer and the birth event.
        return (_horsey.siringWithId == 0) && (_horsey.cooldownEndTime <= now);
    }
    ...
    /// @param _horse A reference to the Horse in storage which needs its timer started.
    function _triggerCooldown(Horse storage _horse) internal {
        // Compute the end of the cooldown time (based on current cooldownIndex)
        _horse.cooldownEndTime = uint64(now + cooldowns[_horse.cooldownIndex]);

        // Increment the breeding count, clamping it at 13, which is the length of the
        // cooldowns array. We could check the array size dynamically, but hard-coding
        // this as a constant saves gas. Yay, Solidity!
        if (_horse.cooldownIndex < 13) {
            _horse.cooldownIndex += 1;
        }
    }
    ...
    ///  period has passed.
    function _isReadyToGiveBirth(Horse _matron) private view returns (bool) {
    ...
    /// @param _horseId reference the id of the newHorse, any user can inquire about it
    ...
    {
        require(_horseId > 0);
        Horse storage kit = horses[_horseId];
    ...
    function _breedWith(uint256 _matronId, uint256 _sireId) internal {
        ...
        Pregnant(HorseIndexToOwner[_matronId], _matronId, _sireId);
    }
    ...
    /// @return The Horse ID of the new newHorse.
    /// @dev Looks at a given Horse and, if pregnant and if the gestation period has passed,
    ///  combines the genes of the two parents to create a new newHorse. The new Horse is assigned
    ///  to the current owner of the matron. Upon successful completion, both the matron and the
    ///  new newHorse will be ready to breed again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new newHorse always goes to the mother's owner.
        ...
        returns(uint256)
    {
        ...
        // Make the new newHorse!
        address owner = HorseIndexToOwner[_matronId];
        uint256 newHorseId = _createHorse(_matronId, matron.siringWithId, parentGen + 1, childGenes, owner);

        // Clear the reference to sire from the matron (REQUIRED! Having siringWithId
        // set is what marks a matron as being pregnant.)
        delete matron.siringWithId;

        // return the new newHorse's ID
        return newHorseId;
    }
}
