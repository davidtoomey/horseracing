...
contract HorseMinting is HorseAuction {
    ...
    function createPromoHorse(uint256 _genes, address _owner) public onlyCOO {
        if (_owner == address(0)) {
             _owner = cooAddress;
        }
        ...
        promoCreatedCount++;
        gen0CreatedCount++;
        _createHorse(0, 0, 0, _genes, _owner);
    }
    ...
    function createGen0Auction(uint256 _genes) public onlyCOO {
        ...
        uint256 horseId = _createHorse(0, 0, 0, _genes, address(this));
        _approve(horseId, saleAuction);
